...
// Vercel deployment fix: Force sticky headers to be visible and interactive
function forceStickyHeadersVisible() {
    ...
    trackHeadings.forEach(heading => {
        ...
        heading.style.zIndex = '999';
        heading.style.position = 'fixed';
        heading.style.top = '60px';
        heading.style.backgroundColor = '#f9f9f9';
        heading.style.webkitTransform = 'translate3d(0, 0, 0)';
        heading.style.transform = 'translate3d(0, 0, 0)';
        heading.style.webkitBackfaceVisibility = 'hidden';
        heading.style.backfaceVisibility = 'hidden';
    });
    
    // Force selectors to be interactive
    const selects = document.querySelectorAll('.sticky-header select');
...
// Run the fix immediately and on DOM content loaded
forceStickyHeadersVisible();
document.addEventListener('DOMContentLoaded', forceStickyHeadersVisible);
window.addEventListener('load', forceStickyHeadersVisible);

// Also run periodically to catch any dynamic content
setInterval(forceStickyHeadersVisible, 1000);